...
def get_data_train(FILE_NAME_TRAIN):
    ...
    tweet_df = pd.read_csv(FILE_NAME_TRAIN, header=None, usecols=[3,4], 
                        names=column_names)
    ...
    train_y = np.append(np.ones((len(positive_tweets), 1)), np.zeros((len(negative_tweets), 1)), axis=0)

    return train_x, train_y


def get_data_test(FILE_NAME_TEST):
    tweet_test_df = pd.read_csv(FILE_NAME_TEST, header=None, usecols=[0,3], names=['id', 'text'])
